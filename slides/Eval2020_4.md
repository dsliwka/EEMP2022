...
rf.fit(X,y)
```
- Note that predicting, calculating the test MSE and performing GridSearchCV is done the same way as usual. In particular, we do not need to specify "scoring" when we classify.
- If we wish to get the feature importance, we just execute the following code:
```
rf.feature_importances_*100
```
