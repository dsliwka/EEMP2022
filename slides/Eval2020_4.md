...
Note that we cover regression trees here. The intuiton holds for classification as well. The only major distinction is that instead predicting the mean in a region $R_j$, we predict the mode.
...
#
6. Import the default data.
7. Draw a random sample of 1000 observations and fit a decision tree classifier with a maximum depth of three on the sample.
8. Display the three and make sure you understand the structure of it.
9. Now perform a Gridsearch 5-fold CV to find the optimal *random forest* classifier where your parameter grid is given as follows:
	```
	param_grid = {'n_estimators': [100,300,500], 'max_features': 'auto'}
	```
10. Get the test error estimate of the best model.
...
# Remark 1: Regresion: Out-of-sample $R^2$

The squared error is not the most inutitive of measures.
...
\begin{equation}
R^2 = 1 - \frac{SSR}{SST} = 1 - \frac{\sum_{i=1}^{N}(y_i - \hat{f}(x_i))^2}{\sum_{i=1}^{N}(y_i - \bar{y})^2} = 1 - \frac{N*MSE}{SST}
\end{equation}
...
- Solution: Instead of using `GridSearchCV`, we make use of `RandomizedSearchCV`:
	- We do not fit each possible candidate model, but fit $n$ models consisting of randomly chosen hyperparamters from our defined parameter grid.
...
1. Estimate the conditional probabilities over the classes at the point $x_i$.
...
\begin{equation}
L(Y_i,\hat{p}(X_i))=-\frac{1}{N}\Bigl(Y_i * log(\hat{p}(X_i)) + (1-Y_i) * log(\hat{p}(X_i))\Bigr)
\end{equation}
...
- We could simply check the correlation coeficients, but this is only a linear measure.
- We could check the $\beta$ coeficients of a linear regression, but the model may not be very predictive and the measure is also limited.
...
<center>
![](./figures/20.png){ width=50% }
</center>
<center>
![](./figures/21.png){ width=50% }
</center>
...
- Initially, we split the data into a training set (75%) and a test set (25%).
Then we fit a simple linear regression on the training set and get the $R^2$ on the test data.

#
There seems to be a mildly positive relationship, but we are not fitting the data well: $R^2=0.005$
<center>
![](./figures/22.png){ width=95% }
</center>
...
<center>
![](./figures/23.png){ width=95% }
</center>

#
<center>
![](./figures/24.png){ width=95% }
</center>
...
3. Split the data into a training set consisting of 75% of the data and a test set consiting of 25%. (Use random_state=181).

#
4. Fit a linear regression on the data where 'roce' is the dependent variable and the 18 dimensions of management practices are the independent variables. See each of the dimensions' coeficient. Which dimensions appear to be most important to predict ROCE? Finally, get the $R^2$ of the regression on the test sample. Does the predictive ability of the method improve by adding these dimensions? Why/why not?
...
2. Split the data into a training set consisting of 75% of the data and a test set consiting of 25%. (Use random_state=181).
...
4. Evaluate the accuracy on the test sample. Is the Decision Tree a good predicter compared to a "stupid" classifier that always predicts the majority class?
...
- We have various method we can choose from when fitting a model on our data. Some are quite inflexible (e.g. linear regression) and some are very flexible (e.g. 1NN regression).
